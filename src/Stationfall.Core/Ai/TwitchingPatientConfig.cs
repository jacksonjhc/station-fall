...
public record TwitchingPatientConfig(
	...
	// Idle stutter-wander: alternate between a short drift in a random direction
	// and a stationary pause. Sells the "twitching" name better than holding still.
	float IdleStutterMinSeconds = 0.4f,
	float IdleStutterMaxSeconds = 1.2f,
	float IdlePauseMinSeconds = 0.5f,
	float IdlePauseMaxSeconds = 2.0f
)
{
